...
static inline bool generate_type_decl(AstNode *node, Buffer *code);
...
static inline void generate_decl(AstNode *node, Buffer *code)
{
  if (generate_type_decl(node, code)) return;
...
static inline bool generate_type_decl(AstNode *node, Buffer *code)
{
  if (node->kind != AST_NODE_KIND_TYPE_DECL) return false;
  AstNonLeafNode *typeDecl = (AstNonLeafNode *) node;
  AstNode *ident = typeDecl->children[0];
  // TODO: Implement code generation for generic type declaration.
  AstNode *typeParams = typeDecl->children[1];
  (void) typeParams;
...
static inline bool generate_struct_decl(AstNode *node, Buffer *code)
{
  ...
  // TODO: Implement code generation for generic struct declaration.
  AstNode *typeParams = structDecl->children[1];
  (void) typeParams;
